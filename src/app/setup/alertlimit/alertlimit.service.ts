...
export class AlertLimitService {
  ...
  getAlertLimit() {
    return this.http.get(this.comcom.commonroute+'alertlimitapi.php')
    .map((res:Response) => res.json());
  }

  updateAlertLimit(catego) {
    let headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' });
    let options = new RequestOptions({ headers: headers });
    let body = JSON.stringify(catego);
    return this.http.put(this.comcom.commonroute+'alertlimitapi.php', body, options )
    .map((res: Response) => res.json());
  }
