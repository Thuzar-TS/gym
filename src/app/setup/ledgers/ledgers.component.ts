...
@Component({
  ...
  providers: [LedgersService,commonRouteUrl,DatePipe]
})
export class LedgersComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,private ledgService: LedgersService, private userdata: commonRouteUrl, private router: Router, private datePipe:DatePipe) { }
  ...
  public toInt(num: string) {
    ...
    dailySave(){
      ...
      if((this.dailyaction != "" && this.dailyaction != null) || (this.dailyfood != "" && this.dailyfood != null)){
        ...
        return this.ledgService.createDaily(dailysave,this.mbrid).subscribe(data => {
          if(data.ledger != "No Record") {
            this.data = data.ledger;
            this.data1 = data.ledger;
          }
      ...
      var incdate = this.datePipe.transform(value,"MM/dd/yyyy");
           this.data = Object.assign([], this.data1).filter(
            //  item => item.category_name.toLowerCase().indexOf(this.offdatefilter.toLowerCase()) > -1
             item => item.income_date.indexOf(incdate) > -1            
           )
       }      
    }
}
